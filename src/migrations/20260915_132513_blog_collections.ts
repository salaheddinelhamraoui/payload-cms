...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  `)
  await db.run(sql`CREATE INDEX \`authors_slug_idx\` ON \`authors_locales\` (\`slug\`,\`_locale\`);`)
  ...
  // Hand-added: carry existing alt text into the new localised table before
  // the old column goes away. `alt` was a plain column and is now localised,
  // and the generated migration creates `media_locales` empty — so without
  // this, every image already uploaded loses its alt text silently. Existing
  // values are the Dutch original by definition, since that was the only
  // locale the flat column could hold.
  await db.run(
    sql`INSERT INTO \`media_locales\` (\`alt\`, \`_locale\`, \`_parent_id\`) SELECT \`alt\`, 'nl', \`id\` FROM \`media\` WHERE \`alt\` IS NOT NULL;`,
  )
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  // Hand-added: rebuild the flat column and refill it from the Dutch values
  // before the localised table is dropped, mirroring the `up` above. SQLite
  // rejects `ADD COLUMN ... NOT NULL` with no default outright, so the
  // generated statement at the end of this function could never have run —
  // it is replaced here rather than left as a rollback that fails halfway.
  await db.run(sql`ALTER TABLE \`media\` ADD \`alt\` text NOT NULL DEFAULT '';`)
  await db.run(
    sql`UPDATE \`media\` SET \`alt\` = COALESCE((SELECT \`alt\` FROM \`media_locales\` WHERE \`media_locales\`.\`_parent_id\` = \`media\`.\`id\` AND \`media_locales\`.\`_locale\` = 'nl'), '');`,
  )
